...
- **支持首次发布**：仓库没有历史版本时先保留发布草稿，等待所有平台构建和完整更新清单准备就绪后再公开。正常升级流程、标签触发和手动触发保持不变。
- **独立的 fork 更新源**：自动更新清单及备用下载链接改为 posy-storage/cockpit-tools，并使用本 fork 的独立更新签名公钥。

### 包含

- 包含 1.3.59 新增的按实例隐藏 Codex 官方额度提示功能：API Service 桌面启动默认开启，其他启动默认关闭，支持保存手动选择和可恢复的语义隐藏。

## [1.3.59] - 2026-09-22

### 新增

- **按实例控制 Codex 官方额度提示**：账号、供应商、API 服务和实例启动预览均新增显示开关，包含默认实例。API Service 桌面启动默认开启，其他启动默认关闭；用户手动选择按实例保存。CLI 不执行显示注入，实例导入/导出不包含该设置。
- **可恢复的语义隐藏**：复用现有 loopback CDP 生命周期，只隐藏输入区域内已确认的 `workspace_member_credits_depleted` 提示，未知提示和安全警告保持可见。关闭开关、停止注入或宿主心跳中断后恢复原始显示，支持 React 重绘和 Composer 替换。

### 修复

- 为全部 18 个语言文件补齐开关名称和说明，并增加偏好持久化、默认值、React Fiber 识别、显示恢复及 CDP 生命周期的回归验证。
